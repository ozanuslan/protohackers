...
Each request, and each response, is a single UDP packet. There are only two types of request: **insert** and **retrieve**.
...
* `foo=` will insert a key `foo` with value `""` (the empty string).
